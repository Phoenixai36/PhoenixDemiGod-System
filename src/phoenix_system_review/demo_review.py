...
from .models.data_models import (
    Component, ComponentCategory, ComponentStatus, Priority, TaskStatus,
    EvaluationResult, Gap, TODOTask, ImpactLevel, Issue, AssessmentResults
)
...
class PhoenixHydraReviewDemo:
    ...
    def discover_phoenix_components(self) -> List[Component]:
        ...
        components.append(Component(
            ...
            configuration={"api_url": "https://sea-turtle-app-nlak2.ondigitalocean.app/v1/"}
        ))
        ...
        # Documentation
        doc_files = ["README.md", "PROJECT-COMPLETION-STATUS.md", "PHOENIX-HYDRA-STATUS-REPORT.md"]
        ...
        if doc_count > 0:
            components.append(Component(
                ...
                path="docs/",
                status=ComponentStatus.OPERATIONAL if doc_count >= 2 else ComponentStatus.DEGRADED,
    ...
    def _evaluate_infrastructure_component(self, component: Component) -> EvaluationResult:
        ...
        completion = len(criteria_met) / (len(criteria_met) + len(criteria_missing)) * 100
        ...
        return EvaluationResult(
            ...
            quality_score=85.0 if component.status == ComponentStatus.OPERATIONAL else 60.0,
            issues=issues
        )
    
    def _evaluate_monetization_component(self, component: Component) -> EvaluationResult:
    ...
    def _evaluate_documentation_component(self, component: Component) -> EvaluationResult:
    ...
    def _evaluate_testing_component(self, component: Component) -> EvaluationResult:
        ...
        criteria_missing = ["comprehensive_coverage", "integration_tests", "performance_tests"]
        ...
        for result in self.evaluation_results:
            for missing_criteria in result.criteria_missing:
                gap = Gap(
                    ...
                    impact=ImpactLevel.HIGH if result.has_critical_issues else ImpactLevel.MEDIUM,
                    effort_estimate=self._estimate_effort(missing_criteria),
                    category=result.component.category,
                    priority=Priority.CRITICAL if result.has_critical_issues else Priority.HIGH
                )
                gaps.append(gap)
...
def main():
    ...
    print("\n🔥 Critical Tasks:")
    critical_tasks = [t for t in assessment.prioritized_tasks if t.priority == Priority.CRITICAL]
    for task in critical_tasks[:3]:
        print(f"   • {task.title} ({task.effort_hours}h)")
    
    print("\n⚡ High Priority Tasks:")
    high_tasks = [t for t in assessment.prioritized_tasks if t.priority == Priority.HIGH]
