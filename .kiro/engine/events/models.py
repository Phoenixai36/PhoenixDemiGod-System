...
class EventFilter(BaseModel):
    ...
    def matches(self, event: BaseEvent) -> bool:
        ...
        if self.filter_type == EventFilterType.SOURCE:
            ...
            result = event.type == self.value if not isinstance(self.value, list) else event.type in self.value
        elif self.filter_type == EventFilterType.SEVERITY:
            result = event.severity == self.value if not isinstance(self.value, list) else event.severity in self.value
        elif self.filter_type == EventFilterType.ATTRIBUTE and self.attribute_name:
            # Simplified attribute matching for brevity
            result = getattr(event, self.attribute_name, None) == self.value
        elif self.filter_type == EventFilterType.CUSTOM and self.custom_filter:
...
class EventFilterGroup(BaseModel):
    ...
    filters: List[Union[EventFilter, "EventFilterGroup"]]
    operator: str = "AND"
    ...
    @classmethod
    def from_dict(cls, data: Optional[Dict[str, Any]]) -> Optional["EventFilterGroup"]:
        if not data:
            return None
        ...
        for f_data in filters_data:
            if "operator" in f_data:
                filters.append(EventFilterGroup.from_dict(f_data))
            else:
                filters.append(EventFilter(**f_data))
        
        return cls(filters=filters, operator=data.get("operator", "AND"))
